...
const Loading = ({ className = "text-white" }) => {
  return (
    <svg className={`-ml-1 mr-2 h-5 w-5 animate-spin ${className}`} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
  ...
  if (type === "outlined") {
    return (
      <button onClick={onClick} disabled={disabled} className={`btn-outlined ${disabled ? "opacity-50" : ""} ${className}`}>
        {isLoading && <Loading className="text-sky-500" />}
        {children}
      </button>
    );
  }
